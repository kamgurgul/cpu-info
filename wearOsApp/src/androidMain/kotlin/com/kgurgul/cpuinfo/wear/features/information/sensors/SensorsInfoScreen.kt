...
@Composable
fun WearSensorsInfoScreen(viewModel: SensorsInfoViewModel = koinViewModel()) {
    val uiState by viewModel.uiStateFlow.collectAsStateWithLifecycle()
    WearSensorsInfoScreen(uiState = uiState)
}

@Composable
fun WearSensorsInfoScreen(uiState: SensorsInfoViewModel.UiState) {
    val columnState =
    ...
    ScreenScaffold(scrollState = columnState) {
        ScalingLazyColumn(columnState = columnState) {
            ...
            items(uiState.sensors, key = { sensorData -> sensorData.id }) { sensorData ->
                WearCpuChip(label = sensorData.name.asString(), secondaryLabel = sensorData.value)
            }
        }
    }
}
